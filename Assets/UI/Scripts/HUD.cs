...
public class HUD : UIManager.UIBehaviour
{
    ...
    public void ModifyHUD()
    {
        ...
        for (int i = 0; i < PlayerHealth.Instance.NumMaxHealthSegments; i++)
        {
            ...
            midTransform.sizeDelta = new Vector2(PlayerHealth.Instance.MaxHealthSegmentSize, midTransform.sizeDelta.y);
    ...
    private void UpdateHUD()
    {
        for (int i = 0; i < PlayerHealth.Instance.NumMaxHealthSegments; i++)
        {
            var segment = PlayerHealth.Instance.HealthSegments[i];
