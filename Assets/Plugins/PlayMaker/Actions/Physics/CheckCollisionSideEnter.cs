...
public class CheckCollisionSideEnter : FsmStateAction
{
	...
	public new void DoCollisionEnter2D(Collision2D collision)
	{
		if (!otherLayer)
		{
			if (LayerMask.LayerToName(collision.gameObject.layer) == "Geometry")
			{
	...
	private void CheckTouching(LayerMask layer)
	{
		...
		{
			RaycastHit2D raycastHit2D = Physics2D.Raycast((Vector3)topRay, Vector2.up, RAYCAST_LENGTH, 1 << (int)layer);
		...
		{
			RaycastHit2D raycastHit2D2 = Physics2D.Raycast((Vector3)rightRay, Vector2.right, RAYCAST_LENGTH, 1 << (int)layer);
		...
		{
			RaycastHit2D raycastHit2D3 = Physics2D.Raycast((Vector3)bottomRay, Vector2.down, RAYCAST_LENGTH, 1 << (int)layer);
		...
		{
			RaycastHit2D raycastHit2D4 = Physics2D.Raycast((Vector3)leftRay, Vector2.left, RAYCAST_LENGTH, 1 << (int)layer);
