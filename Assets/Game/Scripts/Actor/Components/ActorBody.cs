...
public class ActorBody : MonoBehaviour
{
    ...
    void VerticalCollisions(ref Vector2 delta){
        ...
        collisions.below = collisions.above = collisions.leftEdge = collisions.rightEdge = false;
        for (int i = 0; i < verticalRayCount; i++){
            if (verticalHits[i]){
                ...
                if (directionY == 1)
                    collisions.aboveCollider.Add(verticalHits[i].collider);
                if (directionY == -1)
                    collisions.belowColliders.Add(verticalHits[i].collider);
    ...
    public struct CollisionInfo{
        ...
        public List<Collider2D> aboveCollider;
        public List<Collider2D> belowColliders;
        // public List<Collider2D> leftCollider;
        // public List<Collider2D> rightCollider;
